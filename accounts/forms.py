...
class SignUpForm(UserCreationForm):
    """User sign up form with email and password verification using django's
    built-in template, using a verify_email extension"""

    # Add field not in User default form
    verify_email = forms.EmailField(label='Please verify your email',
                                    required=True)

    # sign-up form using builtin and pass1&2 from django forms and extra
    # field from above
    class Meta:
        model = models.User
        fields = [
    ...
    # get cleaned data and both emails from cleaned data for validation
    def clean(self):
        ...
        if email != verify:
            raise forms.ValidationError(
                "Oops! Make sure your emails are matching"
            )


# edit profile form, includes email verification, excludes password
class EditUserForm(forms.ModelForm):
    """User profile form once signed in to update additional profile fields"""
...
class EditPasswordForm(forms.Form):
    ...
    # Override the form's __init__ method to take an extra keyword argument, request.
    # This stores the request in the form, where it's required,
    # and from where you can access it in your clean method.
